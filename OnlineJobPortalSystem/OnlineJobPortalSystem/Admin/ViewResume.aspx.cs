...
namespace OnlineJobPortalSystem.Admin
{
    public partial class ViewResume : System.Web.UI.Page
    {
        ...
        private void ShowAppliedJob()
        {
            ...
            query = @"Select Row_Number() over(Order by (Select 1)) as [Sr.No],aj.AppliedJobs,j.CompanyName,aj.JobId,j.Title,u.Mobile,
        ...
        protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(str);
                cmd = new SqlCommand("Delete from AppliedJobs where AppliedJobs = @id", con);
                cmd.Parameters.AddWithValue("@id", appliedjobId);
                con.Open();
                int r = cmd.ExecuteNonQuery();
                if (r > 0)
                {
                    lblMsg.Text = "Resume Deleted!";
                    lblMsg.CssClass = "alert alert-success";
                }
                else
                {
                    lblMsg.Text = "Cannot Delete Job!";
